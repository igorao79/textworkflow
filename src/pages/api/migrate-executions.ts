...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!fs.existsSync(EXECUTIONS_FILE)) {
      ...
      return res.status(200).json({ success: true, message: 'No executions file found, nothing to migrate' });
    }
    ...
    res.status(200).json({
      success: true,
      message: `Migrated ${executions.length} executions with their logs`
    });
