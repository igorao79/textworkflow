...
- **Задачи добавляются в очередь** для последовательной обработки
- Поддержка retry и timeout

**Development режим:**
- Используется node-cron как fallback
- **Автоматическая обработка очереди** после добавления задачи
...
**Обработка очереди:**
- В production: задачи накапливаются в очереди, обрабатываются вручную или по расписанию
- В development: задачи обрабатываются автоматически сразу после добавления
- Ручная обработка: `POST /api/queue/process`
